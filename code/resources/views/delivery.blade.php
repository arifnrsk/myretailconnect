        ...
        <div class="row review-main-content">
            ...
            <div class="container rcm-container mt-4">
                ...
                <div class="row mb-4 align-items-center" style="width: 95%; height: 120px; background-color: white; border-radius: 10px;">
                    ...
                    <div class="col-2">
                        <!-- Assign Courier Name -->
                        <select class="form-select" id="courierSelect" onchange="updateCourier({{ $delivery->id }}, this.value)">
                            <option value="">Assign Courier</option>
                            @foreach ($couriers as $courier)
                                <option value="{{ $courier->id }}" {{ $delivery->courier_id == $courier->id ? 'selected' : '' }}>
                                    {{ $courier->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-2">
                        <!-- Delivery date -->
                        {{ $delivery->delivery_date->format('M d, Y') }} at {{ $delivery->delivery_date->format('h:i A') }}
                    </div>
                    <div class="col-2">
                        <!-- Delivery type -->
                    ...
                    <div class="col-2">
                        <!-- Assign Status -->
                        <select class="form-select" id="statusSelect" onchange="updateStatus({{ $delivery->id }}, this.value)">
                            <option value="">Choose a delivery status</option>
                            @foreach ($deliveryStatuses as $status)
                                <option value="{{ $status->id }}" {{ $delivery->delivery_status_id == $status->id ? 'selected' : '' }}>
                                    {{ $status->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-2">
                        <div class="col-3 d-flex flex-column align-items-center w-100">
                            <button class="cancel-delivery-btn mb-2" data-delivery-id="{{ $delivery->id }}">
                                Cancel
                            </button>
                            <a href="{{ route('invoices.show', ['id' => $delivery->id]) }}" style="width: 70%">
...
<script>
    ...
    function updateCourier(deliveryId, courierId) {
        fetch(`/delivery/${deliveryId}/update-courier`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
