        ...
        <div class="row review-main-content">
            ...
            <div class="container rcm-container mt-4">
                ...
                <div class="row mb-4 align-items-center" style="width: 95%; height: 120px; background-color: white; border-radius: 10px;">
                    ...
                    <div class="col-2">
                        {{ $transaction->transaction_date->format('M d, Y') }} at {{ $transaction->transaction_date->format('h:i A') }} <!-- Transaction date -->
                    </div>
                    <div class="col-2">
                        {{ $transaction->transactionStatus->name }} <!-- Transaction status -->
                    </div>
                    <div class="col-2">
                        Rp {{ number_format($transaction->total_amount, 2, ',', '.') }} <!-- Total Amount -->
                    </div>
                    <div class="col-2">
                        <div class="col-3 d-flex flex-column align-items-center w-100">
                            <a href="{{ route('customer.invoices.show', ['id' => $transaction->invoice->id]) }}" style="width: 70%;">
